...
// Hàm gọi tới Backend Vercel để lấy câu thoại từ Gemini
async function GhetBotChat() {
    try {
        const res = await fetch('/api/chat', { method: 'POST' });
        const data = await res.json();
        if (data.reply) {
            document.getElementById('bot-dialog').innerText = "Bot: " + data.reply;
        }
    } catch (err) {
        console.error("Không gọi được AI");
    }
}
...
document.addEventListener('mousedown', (e) => {
    ...
    if (intersects.length > 0) {
        ...
        // Kích hoạt gọi Gemini AI khi Bot bị bắn trúng
        document.getElementById('bot-dialog').innerText = "Bot: Đang suy nghĩ...";
        GhetBotChat();
    }
});
